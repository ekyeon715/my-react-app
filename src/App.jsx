  ...
  return (
    <main className='container'>
      ...
      <div className='inner-box'>
        ...
        <button className='button' onClick={addCountryHandler}>국가추가</button>
        <button className='button' onClick={updateCountryHandler}>업데이트</button>
      </div>

      <table border="0">
        <thead>
          <tr><th>국가별</th><th>금메달</th><th>은메달</th><th>동메달</th><th>액션</th></tr>
        </thead>
        <tbody>
          {countries.map((c) => (
            <tr key={c.id}>
              ...
              <td>{c.silver}</td>
              <td>{c.bronze}</td>
              <td><button onClick={e => deleteCountryHandler(c.id)}>삭제</button></td>
            </tr>
          ))}
        </tbody>
      </table>
